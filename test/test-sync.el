...
(describe "Sync Tests"
  ...
  (it "Creating sync event for inserting text kvd"
    (let* ((pkm-sync-add-event-func (lambda (event)
      ...
      (expect (pkm-get-db-id inserted-kvd) :to-equal (pkm-get-db-id kvd))
  ...
  (it "Test deleting kvd-link"
    (let* ((pkm-sync-add-event-func (lambda (event)
      ...
      (expect (pkm-get-db-id inserted-kvd) :to-equal (oref (car kvds) :id))
      (expect (pkm-get-db-id inserted-link) :to-equal (oref (car kvds) :id))
      (pkm2--db-delete-link-between-node-and-kvd (pkm-get-db-id inserted-link)
                                                 type)
  ...
  (it "Test archiving kvd-link"
    (let* ((pkm-sync-add-event-func (lambda (event)
      ...
      (expect (pkm-get-db-id inserted-kvd) :to-equal (oref (car kvds) :id))
      (expect (pkm-get-db-id inserted-link) :to-equal (oref (car kvds) :id))
      (pkm2-db-archive-link-between-node-and-kvd (pkm-get-db-id inserted-link)
                                                 type
                                                 (pkm2-get-current-timestamp))
      (expect (length (plist-get events :main)) :to-be 4)
      (expect (-map (lambda (event) (plist-get event :action)) (plist-get events :main))
