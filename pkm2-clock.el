...
(pkm2-register-behavior-2 (behavior-schema
                           :name "clock"
                           :assets (list
                                    (kvd-schema  :name "clock-start" :key "clock-start" :value #'pkm2-get-user-selected-timestamp
                                                 :data-type 'DATETIME )
                                    (kvd-schema :name "clock-end" :key "clock-end" :value #'pkm2-get-user-selected-timestamp
                                                :data-type 'DATETIME :managed t))
                           :groups (list
                                    (kvd-group-schema :kvds (list "clock-start" "clock-end") :name "clock-group")
                                    )))

(pkm-register-structure-2
 (object-schema :parent-schema-name "base-n" :name "clock-n"
                ...
                :is-behavior "clock"
                :behaviors (list `(:name "clock" :link-to (primary)))))
